...
async def get_history_marcketing(user_id: str):
    """히스토리 조회"""
    try:
        data = requests.get(
            url=f"http://{BACKEND_URL}:{BACKEND_PORT}/bj/hart-history/bot/{user_id}",
            timeout=5,
        )
        return data.json()
    except:  # pylint: disable=W0702
...
async def request_delete_view_bot(panda_id: str):
    """백엔드에 게스트 요청"""
    try:
        requests.delete(
            url=f"http://{BACKEND_URL}:{BACKEND_PORT}/resource/views/{panda_id}",
            timeout=5,
        )
    except:  # pylint: disable=W0702
        return None
