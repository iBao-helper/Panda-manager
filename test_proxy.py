...
async def main():
    """docstring"""
    # api_client = APIClient(panda_id="siveriness00")
    # await api_client.dump()
    for ip in ip_lists:
        print(f"ip: {ip}")
        local_api_client = APIClient(panda_id="siveriness00")
        await local_api_client.login(
            login_id="siveriness00", login_pw="dkflfkd12#", panda_id="mickey94"
        )
        local_api_client.proxy_ip = ip
        await local_api_client.play("mickey94")
